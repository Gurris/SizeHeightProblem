...
class ShiftDetailTableViewCell: UITableViewCell {
    ...
    var timeblock: ShiftDetailTimeblockViewModel?
    weak var cellDelegate: CollectionViewCellDelegate?
    
    override func awakeFromNib() {
        super.awakeFromNib()
        
        let layout = UICollectionViewFlowLayout() //UICollectionViewCenterLayout() //UICollectionViewFlowLayout()
        ...
        let cellNib = UINib(nibName: "ShiftDetailCollectionViewCell", bundle: nil)
...
extension ShiftDetailTableViewCell: UICollectionViewDelegate, UICollectionViewDataSource{
    ...
    func updateCellWith(timeblock: ShiftDetailTimeblockViewModel) {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return self.timeblock?.tags.count ?? 0
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "collectionviewcellid", for: indexPath) as? ShiftDetailCollectionViewCell{
            ...
            cell.tagName.text = self.timeblock?.tags[indexPath.item].tagName
